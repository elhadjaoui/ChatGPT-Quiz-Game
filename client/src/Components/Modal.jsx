...
const Modal = () => {
  const { closeModal, isModalOpen, correct, questions, setShowResults } = useGlobalContext();
      ...
      {isModalOpen && (
        <div className="absolute top-0 left-0 h-screen w-full flex items-center bg-[rgba(0,0,0,.5)]">
          ...
          <div className=" text-center bg-white p-8 mx-auto rounded-lg max-w-[600px] w-11/12">
            ...
            <button
              className="btn bg-green-500 hover:bg-green-600 mr-2 mt-2 "
              onClick={closeModal}
            >
              Play Again
            </button>
            <button
              className="btn bg-green-500 hover:bg-green-600 mt-2"
              onClick={() => {
                closeModal();
                setShowResults(true);
              }}
